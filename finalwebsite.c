...
//text = "Your current grade is equal to %f\n", finalgrade / 100; 
 fprintf(f,"Your current grade is equal to %f\n", finalgrade / 100);
 if (num == 3)
 {
...
//		 printf("%d\n", aw);
	...
	if ( num == 2 )
	{
		...
		fprintf(f,"You need to get %f points out of %f points on your last assignment to get a %f in the class\n", x, fk, dg);
		float NesGradeCat2()
		{
			return(100*(x/fk));
		}
		...
		fprintf(f,"You need a %f percent on your last assignment to get a %f percent in the class\n",g, dg);
	}
	if ( num == 3 )
		{
			float NesPercCat3()
